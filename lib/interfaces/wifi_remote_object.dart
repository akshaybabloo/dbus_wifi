...
class OrgFreedesktopNetworkManager extends DBusRemoteObject {
  OrgFreedesktopNetworkManager(DBusClient client, String destination,
      {DBusObjectPath path = const DBusObjectPath.unchecked('/org/freedesktop/NetworkManager')})
  ...
  Future<List<DBusObjectPath>> callGetAllDevices(
    ...
    var result = await callMethod('org.freedesktop.NetworkManager', 'GetAllDevices', [],
  ...
  Future<(DBusObjectPath path, DBusObjectPath activeConnection)> callAddAndActivateConnection(
      Map<String, Map<String, DBusValue>> connection, DBusObjectPath device, DBusObjectPath specificObject,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod(
        ...
        [
          DBusDict(DBusSignature('s'), DBusSignature('a{sv}'),
              connection.map((key, value) => MapEntry(DBusString(key), DBusDict.stringVariant(value)))),
          device,
          specificObject
        ],
  ...
  Future<List<DBusObjectPath>> getAccessPoints() async {
    var value = await getProperty('org.freedesktop.NetworkManager.Device.Wireless', 'AccessPoints',
  ...
  Future<DBusObjectPath> getActiveAccessPoint() async {
    var value = await getProperty('org.freedesktop.NetworkManager.Device.Wireless', 'ActiveAccessPoint',
  ...
  Future<int> getMode() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.Device.Wireless', 'Mode', signature: DBusSignature('u'));
    return value.asUint32();
  }

  /// Invokes org.freedesktop.NetworkManager.Device.Wireless.GetAllAccessPoints()
  Future<List<DBusObjectPath>> callGetAllAccessPoints(
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod('org.freedesktop.NetworkManager.Device.Wireless', 'GetAllAccessPoints', [],
        replySignature: DBusSignature('ao'),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
    return result.returnValues[0].asObjectPathArray().toList();
  ...
  Future<void> callRequestScan(Map<String, DBusValue> options,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.NetworkManager.Device.Wireless', 'RequestScan', [DBusDict.stringVariant(options)],
  ...
  Future<String> getHwAddress() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.AccessPoint', 'HwAddress', signature: DBusSignature('s'));
  ...
  Future<int> getStrength() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.AccessPoint', 'Strength', signature: DBusSignature('y'));
    return value.asByte();
  }
  ...
  Future<int> getWpaFlags() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.AccessPoint', 'WpaFlags', signature: DBusSignature('u'));
    return value.asUint32();
  }
  ...
  Future<int> getRsnFlags() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.AccessPoint', 'RsnFlags', signature: DBusSignature('u'));
    return value.asUint32();
  }
  ...
  Future<int> getLastSeen() async {
    var value =
        await getProperty('org.freedesktop.NetworkManager.AccessPoint', 'LastSeen', signature: DBusSignature('i'));
