...
class OrgFreedesktopNetworkManagerSettingsUpdated extends DBusSignal {
  OrgFreedesktopNetworkManagerSettingsUpdated(DBusSignal signal)
      : super(
...
class OrgFreedesktopNetworkManagerSettingsRemoved extends DBusSignal {
  OrgFreedesktopNetworkManagerSettingsRemoved(DBusSignal signal)
      : super(
            sender: signal.sender,
            path: signal.path,
            interface: signal.interface,
...
class OrgFreedesktopNetworkManagerSettings extends DBusRemoteObject {
  ...
  OrgFreedesktopNetworkManagerSettings(DBusClient client, String destination,
      {DBusObjectPath path = const DBusObjectPath.unchecked('/org/freedesktop/NetworkManager/Settings')})
      : super(client, name: destination, path: path) {
    updated = DBusRemoteObjectSignalStream(
            object: this,
            interface: 'org.freedesktop.NetworkManager.Settings.Connection',
            name: 'Updated',
            signature: DBusSignature(''))
        .asBroadcastStream()
        .map((signal) => OrgFreedesktopNetworkManagerSettingsUpdated(signal));

    removed = DBusRemoteObjectSignalStream(
            object: this,
            interface: 'org.freedesktop.NetworkManager.Settings.Connection',
            name: 'Removed',
            signature: DBusSignature(''))
        .asBroadcastStream()
        .map((signal) => OrgFreedesktopNetworkManagerSettingsRemoved(signal));
  }
  ...
  Future<List<DBusObjectPath>> callListConnections(
    ...
    var result = await callMethod('org.freedesktop.NetworkManager.Settings', 'ListConnections', [],
  ...
  Future<DBusObjectPath> callGetConnectionByUuid(String uuid,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod('org.freedesktop.NetworkManager.Settings', 'GetConnectionByUuid', [DBusString(uuid)],
  ...
  Future<DBusObjectPath> callAddConnection(Map<String, Map<String, DBusValue>> connection,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
  ...
  Future<void> callSaveHostname(String hostname,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.NetworkManager.Settings', 'SaveHostname', [DBusString(hostname)],
  ...
  Future<void> callUpdate(Map<String, Map<String, DBusValue>> properties,
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod(
        ...
        [
          DBusDict(DBusSignature('s'), DBusSignature('a{sv}'),
              properties.map((key, value) => MapEntry(DBusString(key), DBusDict.stringVariant(value))))
        ],
        replySignature: DBusSignature(''),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
  ...
  Future<void> callDelete({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.NetworkManager.Settings.Connection', 'Delete', [],
        replySignature: DBusSignature(''),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<Map<String, Map<String, DBusValue>>> callGetSettings(
      {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod('org.freedesktop.NetworkManager.Settings.Connection', 'GetSettings', [],
